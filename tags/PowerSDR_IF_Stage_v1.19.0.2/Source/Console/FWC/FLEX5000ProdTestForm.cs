...
namespace PowerSDR
{
	unsafe public class FLEX5000ProdTestForm : System.Windows.Forms.Form
	{
		...
		private void TestPreamp()
		{
			...
			switch(console.CurrentModel)
			{
				case Model.FLEX3000: target = 24.5; break;
				case Model.FLEX5000: target = 14.0; break;
			}
			Debug.WriteLine("Preamp Test: "+(a[peak_bin] - peak).ToString("f1")+" dB");
			bool b = !((target - (a[peak_bin] - peak)) > 3.0f ||
				(target - (a[peak_bin] - peak)) < -1.0f); // pass from target +3 or -1
			if(!b)
			{
				btnGenPreamp.BackColor = Color.Red;
				test_preamp = " Preamp Test: Failed delta > "+target.ToString("f1")+" +/-1dB ("+(a[peak_bin]-peak).ToString("f1")+")";
			}
			else
			{
