...
*/
		...
		private static int VACBufSize()
		{
			// return value based on block_size1, block_size2, and sample_rate1
			return 65536; // temporary
		}

		unsafe private static void InitVAC()
		{
			int buf_size = VACBufSize();
			if(rb_vacOUT_l == null) rb_vacOUT_l = new RingBuffer(buf_size);
			rb_vacOUT_l.Restart(block_size2);
			
			if(rb_vacOUT_r == null) rb_vacOUT_r = new RingBuffer(buf_size);
			rb_vacOUT_r.Restart(block_size2);

			if(rb_vacIN_l == null) rb_vacIN_l = new RingBuffer(4*buf_size);
			rb_vacIN_l.Restart(block_size2);

			if(rb_vacIN_r == null) rb_vacIN_r = new RingBuffer(4*buf_size);
			...
			if (sample_rate2 != sample_rate1) 
			{
				...
				if(res_inl == null) res_inl  = new float [4*buf_size];
				if(res_inr == null) res_inr  = new float [4*buf_size];
